...
TEST_F( OBDOverCANModuleTest, OBDOverCANSessionManagerAssertMessageSent_LinuxCANDep )
{
    ...
    receiverOptions.mSourceCANId = ENGINE_ECU_RX;
    receiverOptions.mDestinationCANId = BROADCAST_ID;
...
TEST_F( OBDOverCANModuleTest, OBDOverCANModuleAndDecoderManifestLifecycle_LinuxCANDep )
{
    ...
    engineECUOptions.mSourceCANId = ENGINE_ECU_RX;
    engineECUOptions.mDestinationCANId = ENGINE_ECU_TX;
...
TEST_F( OBDOverCANModuleTest, OBDOverCANModuleRequestVIN_LinuxCANDep )
{
    ...
    ASSERT_TRUE( ecmRxPDUData[0] == vehicleIdentificationNumberRequest.mSID );
...
TEST_F( OBDOverCANModuleTest, OBDOverCANModuleRequestEmissionSupportedPIDsTest_LinuxCANDep )
{
    ...
    transmissionECUOptions.mSourceCANId = TRANSMISSION_ECU_RX;
    transmissionECUOptions.mDestinationCANId = TRANSMISSION_ECU_TX;
    ...
    ASSERT_TRUE( ecmRxPDUData[0] == CURRENT_STATS );
    ...
    // Received Supported PID request for SID 0x01 from FWE
    ASSERT_TRUE( tmcRxPDUData[0] == CURRENT_STATS );
    ...
    ASSERT_TRUE( obdModule.getSupportedPIDs( CURRENT_STATS, ECUType::ENGINE, enginePIDs ) );
    ...
    ASSERT_TRUE( obdModule.getSupportedPIDs( CURRENT_STATS, ECUType::TRANSMISSION, transmissionPIDs ) );
...
TEST_F( OBDOverCANModuleTest, OBDOverCANModuleRequestEmissionPIDDataTest_LinuxCANDep )
{
    ...
    std::map<SignalID, SignalValue> expectedPIDSignalValue = {
        { ENGINE_LOAD, 60 }, { ENGINE_COOLANT_TEMPERATURE, 70 }, { VEHICLE_SPEED, 35 } };
...
TEST_F( OBDOverCANModuleTest, OBDOverCANModuleRequestPIDAndDTCsTest_LinuxCANDep )
{
    ...
    // Received Supported PID request for SID 0x01 from FWE
    ASSERT_TRUE( tcmRxPDUData[0] == CURRENT_STATS );
    ...
    ASSERT_EQ( dtcInfo.mSID, STORED_DTC );
